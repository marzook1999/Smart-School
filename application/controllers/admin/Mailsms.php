...
class Mailsms extends Admin_Controller
{
    ...
    public function compose_sms()
    {
        ...
        $date          = date('Y-m-d');
        $birthDaysList = array();
        $birthStudents = $this->student_model->getBirthDayStudents($date,false,true);
        $birthStaff = $this->staff_model->getBirthDayStaff($date,1,false,true);

        if (!empty($birthStudents)) {
            $array = array();
            foreach ($birthStudents as $student_key => $student_value) {
             
                $array[] = array('name' => $student_value['firstname'] . " " . $student_value['lastname'], 'contact_no' => $student_value['mobileno']);
            }
            $birthDaysList['students']=$array;
        }
        if (!empty($birthStaff)) {
    ...
    public function send_group()
    {
        ...
        $this->form_validation->set_rules('group_send_by', $this->lang->line('message')." ".$this->lang->line('to'), 'required');
        if ($this->form_validation->run()) {
            $user_array = array();
