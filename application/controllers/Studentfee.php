...
class Studentfee extends Admin_Controller
{
     ...
     public function collection_report()
    {
     ...
     if (!$this->rbac->hasPrivilege('collect_fees', 'can_view')) {
        ...
        $data['results']=$this->studentfeemaster_model->getFeeCollectionReport($start_date,$end_date,$received_by,$group);   //die;
        $data['collectlist']=$this->studentfeemaster_model->getFeeCollectionReport($start_date,$end_date);
      
        foreach($data['collectlist'] as $value){

                if($value['received_by']!=''){
                $collect_by[$value['received_by']]=$value['received_byname'];
                }
