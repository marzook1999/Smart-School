...
class Homework_model extends MY_model
{
    ...
    public function get($id = null)
    {
        ...
        if (!empty($id)) {
            $this->db->select("`homework`.*,subject_group_subjects.subject_id,subject_group_subjects.id as `subject_group_subject_id`,subjects.name,subject_groups.id as subject_groups_id,subject_groups.name,(select count(*) as total from submit_assignment where submit_assignment.homework_id=homework.id) as assignments");
    ...
    public function searchHomeworkEvaluation($class_id, $section_id, $subject_id)
    {
        ...
        $query = $this->db->select('homework.*,classes.class,sections.section,subjects.name')
            ...
            ->get('homework');
        //$query =  $this->db->query("select homework.*,classes.class,sections.section,subjects.name from homework inner join classes on (classes.id = homework.class_id) inner join sections on (sections.id = homework.section_id)inner join subjects on (subjects.id = homework.subject_id) where homework.id in (select homework_evaluation.homework_id from homework_evaluation join homework on (homework_evaluation.homework_id = homework.id) group by homework_evaluation.homework_id) ");
    ...
    public function count_students($class_id, $section_id)
    {
        ...
        $query = $this->db->select("student_session.student_id")->join("student_session", "students.id = student_session.student_id")->where(array('student_session.class_id' => $class_id, 'student_session.section_id' => $section_id, 'students.is_active' => "yes"))->group_by("student_session.student_id")->get("students");
